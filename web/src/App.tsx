...
export default function App() {
  ...
  const addFiles = useCallback((newFiles: File[]) => {
    ...
    setFiles((prev) => [
      ...prev,
      ...newFiles.map((f) => ({
        id: fileId(), file: f, name: f.name, size: f.size,
        ext: getExt(f.name), status: "pending" as const,
      })),
    ]);
  }, []);
  ...
  const doStart = async () => {
    ...
    setProgress(0); setCurrentFile(""); setLogsExpanded(true);
    ...
    try {
      ...
      setJobPhase("idle"); setLoadingState("idle");
      setFiles((prev) =>
        prev.map((f) =>
    ...
    try {
      ...
      if (job.status === "success") {
        ...
        job.results.forEach((r) => {
          addHistoryEntry({
            id: r.id, fileName: r.file_name, fileSize: 0, ext: ".md",
            status: "success", resultUrl: r.download_url, previewUrl: r.preview_url,
            timestamp: Date.now(),
          });
        });
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.status === "converting" ? { ...f, status: "failed" as const, error: job.error || undefined } : f
          )
        );
      }
    } catch { /* swallow */ }
  ...
  return (
    <div className={`min-h-screen ${gradientBg} relative overflow-hidden`}>
      {/* Decorative blobs */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] rounded-full pointer-events-none -translate-y-1/2 translate-x-1/4"
        style={{ background: "radial-gradient(circle, rgba(59,130,246,0.08) 0%, transparent 70%)" }} />
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] rounded-full pointer-events-none translate-y-1/2 -translate-x-1/4"
            ...
            <div className={cardNoHover + " p-5"}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                  {lang === "zh" ? "最近转换" : "Recent Conversions"}
                </h3>
                <div className="flex gap-2">
                  <button onClick={clearHistory} className="text-xs text-slate-400 hover:text-rose-500 transition-colors">
                    {lang === "zh" ? "清空" : "Clear"}
                  </button>
                  <button onClick={() => setShowHistory(false)} className="text-xs text-blue-500 hover:text-blue-700 transition-colors">
                  ...
                  {[...history].reverse().slice(0, 10).map((h) => (
                    <div key={h.id} className="flex items-center justify-between px-4 py-2.5 bg-slate-50 rounded-xl text-sm">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="truncate text-slate-700">{h.fileName}</span>
                      </div>
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              showHistory ? "bg-blue-100 text-blue-700" : "bg-white/80 text-slate-500 hover:bg-white hover:text-slate-700 border border-white/70"
            }`}
          >
          ...
          {templateExists && (
            <button
              ...
              className="px-3 py-1.5 rounded-lg text-xs font-medium bg-white/80 text-slate-500 hover:bg-white hover:text-slate-700 border border-white/70 transition-colors"
            >
              {lang === "zh" ? "📁 加载模板" : "📁 Load Template"}
            </button>
          )}
          <button
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-12 lg:col-span-5 flex flex-col gap-5">
            ...
            <div className={`${cardNoHover} p-5`}>
              {showSkeleton ? (
                ...
                <ActionPanel
                  lang={lang} hasPending={hasPending} isConverting={isConverting}
                  hasFiles={files.length > 0} progress={progress} currentFile={currentFile}
                  visible={isConverting || jobPhase === "done"}
                  onStart={handleStart} onClear={clearFiles}
                />
              )}
              {error && (
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-rose-700">
                        {lang === "zh" ? "转换出现问题" : "Something went wrong"}
                      </p>
                      ...
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={handleRetry}
                          disabled={retrying}
                          className="px-3 py-1.5 text-xs font-medium bg-rose-100 text-rose-700 rounded-lg hover:bg-rose-200 transition-colors disabled:opacity-50"
                        >
                          {retrying ? (lang === "zh" ? "重试中…" : "Retrying…") : (lang === "zh" ? "🔄 重试" : "🔄 Retry")}
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            <div className={`${cardNoHover} p-5`}>
              <LogPanel lang={lang} logs={logs} expanded={logsExpanded} onToggle={() => setLogsExpanded(!logsExpanded)} />
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-7 flex flex-col gap-5">
            <div className={`${cardNoHover} p-5`}>
              {showSkeleton ? (
                <div className="space-y-2">
                  <SkeletonFileItem /><SkeletonFileItem /><SkeletonFileItem />
                </div>
              ) : (
                <FileList lang={lang} files={files} onRemove={removeFile} onClear={clearFiles} />
              )}
            </div>
            ...
            <div className={`${cardNoHover} p-5`}>
              {showSkeleton ? (
                <div className="space-y-2">
                  <SkeletonResultItem /><SkeletonResultItem />
                </div>
              ) : (
                <ResultPanel
                  lang={lang} results={results} zipUrl={zipResult?.download_url}
      ...
      <HelpCenter
        ...
        onOpenFeedback={() => { setHelpOpen(false); setFeedbackOpen(true); }}
      />
      ...
      {previewResult && (
        <MarkdownPreview
          lang={lang} content={previewContent}
          fileName={previewResult.file_name}
          onClose={() => { setPreviewResult(null); setPreviewContent(""); }}
        />
      )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-base font-bold text-slate-800">
                {t("overwrite_confirm_title" as any, lang)}
              </h3>
            </div>
            <p className="text-sm text-slate-500 leading-relaxed mb-6">
              {t("overwrite_confirm_desc" as any, lang)}
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setOverwriteConfirmOpen(false)}
                className="flex-1 px-4 py-2.5 text-sm bg-slate-100 text-slate-600 rounded-xl hover:bg-slate-200 transition-colors"
              >
                {t("overwrite_confirm_cancel" as any, lang)}
              </button>
              <button
                onClick={handleOverwriteConfirm}
                className="flex-1 px-4 py-2.5 text-sm font-medium bg-rose-600 text-white rounded-xl shadow-md hover:bg-rose-700 hover:shadow-lg transition-all"
              >
                {t("overwrite_confirm_proceed" as any, lang)}
              </button>
            </div>
          </div>
        </div>
